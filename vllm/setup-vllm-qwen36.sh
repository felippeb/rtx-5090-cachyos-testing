#!/usr/bin/env bash
# Install script for Qwen3.6-27B-NVFP4 on vLLM (CachyOS + RTX 5090)
# Downloads model and installs systemd services
# Run as: sudo bash setup-vllm-qwen36.sh [--update]
...
MODEL_REPO="sakamakismile/Qwen3.6-27B-NVFP4"
...
echo "╔══════════════════════════════════════════════════════════╗"
echo "║  vLLM + Qwen3.6-27B-NVFP4 Installer                     ║"
echo "║  Model: sakamakismile/Qwen3.6-27B-NVFP4                 ║"
echo "╚══════════════════════════════════════════════════════════╝"
echo ""
...
CT_VER=$("$VLLM_VENV/bin/python" -c "import compressed_tensors; print(compressed_tensors.__version__)" 2>/dev/null || echo "MISSING")
ok "compressed-tensors: $CT_VER"
...
for repo in cache.repos:
    if 'Qwen3.6-27B-NVFP4' in repo.repo_id:
        print('cached')
        break
" 2>/dev/null || echo "")
    ...
    if [[ "$cached" == "cached" ]]; then
        ...
        read -r
        HF_XET_HIGH_PERFORMANCE=1 "$HF_CLI" download "$MODEL_REPO" --include "*.json *.jinja"
        ok "Model downloaded"
    fi
fi
...
for svc_base in vllm-qwen3.6-27b-nvfp4-turbo vllm-qwen3.6-27b-nvfp4; do
...
echo ""
info "Start with:"
info "  ./scripts/service-switcher.sh nvfp4        (131K context, compressed-tensors)"
info "  ./scripts/service-switcher.sh nvfp4-turbo  (230K context, compressed-tensors)"
info ""
