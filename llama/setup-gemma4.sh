#!/usr/bin/env bash
# Install script for Gemma 4 31B on CachyOS + RTX 5090
# Targets /opt/ for system-wide installation
# Run as: sudo bash setup-gemma4.sh [--test|--update]
#
# --test    Install to /opt/gemma4-test/ on port 10502 (safe to run alongside prod)
...
if [[ $TEST_MODE -eq 1 ]]; then
    LLAMA_DIR="/opt/gemma4-test"
    MODELS_DIR="/opt/gemma4-test/models"
...
MODEL_DIR="$MODELS_DIR/gemma4-31b"
...
section_model() {
    info "=== Section 3: Download Gemma 4 31B ==="
    ...
    if [[ -f "$MODEL_DIR/$MODEL_NAME" ]]; then
        ...
        info "Downloading $MODEL_NAME (~18 GB)..."
    ...
    if [[ -f "$MODEL_DIR/$MMPROJ_NAME" ]]; then
        ...
        info "Downloading $MMPROJ_NAME (~1.1 GB)..."
...
section_services() {
    ...
    for svc_base in llama-server-gemma4-turbo; do
    ...
    local turbo_svc="llama-server-gemma4-turbo${SERVICE_NAME_SUFFIX}"
    ...
    if [[ $TEST_MODE -eq 1 ]]; then
        ...
        info "  ./scripts/service-switcher.sh gemma4-turbo   (Gemma 4 31B, 131K context)"
...
section_opencode() {
    ...
    info "To use Gemma 4 with OpenCode, add this to ~/.config/opencode/opencode.json:"
    info ""
    cat <<'CONFIG'
  "gemma4": {
...
CONFIG
    info ""
    info 'Then set: "model": "gemma4/gemma4-31b"'
}
...
main() {
    if [[ $UPDATE_MODE -eq 1 ]]; then
        echo "╔══════════════════════════════════════════════════════════╗"
        echo "║  llama.cpp + Gemma 4 31B Updater                        ║"
        ...
        echo "╔══════════════════════════════════════════════════════════╗"
        echo "║  llama.cpp + Gemma 4 31B Installer (TEST MODE)          ║"
        echo "║  Target: /opt/gemma4-test/  Port: 10502                 ║"
        echo "╚══════════════════════════════════════════════════════════╝"
    else
        echo "╔══════════════════════════════════════════════════════════╗"
        echo "║  llama.cpp + Gemma 4 31B Installer for CachyOS          ║"
        echo "║  Target: /opt/llama.cpp + /opt/models/gemma4-31b        ║"
        echo "╚══════════════════════════════════════════════════════════╝"
    fi
    echo ""
    ...
    if [[ $UPDATE_MODE -eq 1 ]]; then
        ok "═══════════════════════════════════════════════════════════"
        ok "Gemma 4 31B Update complete!"
        ok "═══════════════════════════════════════════════════════════"
        echo ""
        info "Restart your server to pick up changes:"
        info "  ./scripts/service-switcher.sh gemma4-turbo"
    else
        ok "═══════════════════════════════════════════════════════════"
        ok "Gemma 4 31B Installation complete!"
        ok "═══════════════════════════════════════════════════════════"
        echo ""
        if [[ $TEST_MODE -eq 1 ]]; then
            info "Test instance:"
            info "  1. Verify:  curl http://localhost:10502/v1/models"
            info "  2. Clean:   sudo systemctl stop --now llama-server-gemma4-turbo-test"
        else
            info "Next steps:"
            info "  1. Verify:        curl http://localhost:10500/v1/models"
            info "  2. Switch model:  ./scripts/service-switcher.sh gemma4-turbo"
