#!/usr/bin/env bash
# Install script for Qwen2.5-VL-7B-Instruct (llama.cpp, vision model)
# Uses official llama.cpp with vision support
# Targets /opt/ for system-wide installation
# Run as: sudo bash setup-qwen2.5-vl-7b.sh [--test|--update]
#
# --test    Install to /opt/qwen2.5-vl-7b-test/ on port 10504 (safe to run alongside prod)
...
if [[ $TEST_MODE -eq 1 ]]; then
    LLAMA_DIR="/opt/qwen2.5-vl-7b-test/llama.cpp"
    MODELS_DIR="/opt/qwen2.5-vl-7b-test/models"
    SERVICE_NAME_SUFFIX="-test"
    SERVER_PORT=10504
else
    TEST_MODE=0
    LLAMA_DIR="/opt/qwen2.5-vl-7b/llama.cpp"
    MODELS_DIR="/opt/qwen2.5-vl-7b/models"
    SERVICE_NAME_SUFFIX=""
    SERVER_PORT=10500
fi

MODEL_DIR="$MODELS_DIR/qwen2.5-vl-7b"
...
section_model() {
    info "=== Section 3: Download Qwen2.5-VL-7B-Instruct ==="
    ...
    if [[ -f "$MODEL_DIR/$MODEL_NAME" ]]; then
        ...
        info "Downloading $MODEL_NAME (~5.2 GB)..."
    ...
    if [[ -f "$MODEL_DIR/$MMPROJ_NAME" ]]; then
        ...
        info "Downloading $MMPROJ_NAME (~15 GB)..."
        HF_XET_HIGH_PERFORMANCE=1 HF_TOKEN="$HF_TOKEN" "$HF_CLI" download "$HF_REPO" "$MMPROJ_NAME" --local-dir "$MODEL_DIR"
    fi
...
section_services() {
    ...
    for svc_base in llama-server-qwen2.5-vl-7b; do
        local src="$CONFIGS_DIR/${svc_base}.service"
        local dest_name="${svc_base}${SERVICE_NAME_SUFFIX}.service"
        if [[ -f "$src" ]]; then
            sed -e "s|/opt/qwen2.5-vl-7b/llama.cpp|$LLAMA_DIR|g" \
                -e "s|/opt/qwen2.5-vl-7b/models|$MODELS_DIR|g" \
    ...
    if [[ -f "/etc/systemd/system/${svc_base}${SERVICE_NAME_SUFFIX}.service" ]]; then
        ...
        sed -i 's|Qwen2.5-VL-7B-Instruct-mmproj-F16.gguf|mmproj-F16.gguf|g' "/etc/systemd/system/${svc_base}${SERVICE_NAME_SUFFIX}.service"
        systemctl daemon-reload
        ok "Service patched"
    fi

    local active_svc="llama-server-qwen2.5-vl-7b${SERVICE_NAME_SUFFIX}"
    info "Starting server (8K context, vision enabled)..."
    ...
    if [[ $TEST_MODE -eq 1 ]]; then
        ...
        info "  ./scripts/service-switcher.sh vl-7b          (Qwen2.5-VL-7B, 8K context)"
    fi
...
section_opencode() {
    ...
    info "To use Qwen2.5-VL-7B with OpenCode, add this to ~/.config/opencode/opencode.json:"
    info ""
    cat <<'CONFIG'
  "qwen2.5-vl-7b": {
...
CONFIG
    info ""
    info 'Then set: "model": "qwen2.5-vl-7b/qwen2.5-vl-7b"'
}
...
main() {
    if [[ $UPDATE_MODE -eq 1 ]]; then
        echo "╔══════════════════════════════════════════════════════════╗"
        echo "║  llama.cpp + Qwen2.5-VL-7B Updater                      ║"
        ...
        echo "╔══════════════════════════════════════════════════════════╗"
        echo "║  llama.cpp + Qwen2.5-VL-7B Installer (TEST MODE)        ║"
        echo "║  Target: /opt/qwen2.5-vl-7b-test/  Port: 10504          ║"
        echo "╚══════════════════════════════════════════════════════════╝"
    else
        echo "╔══════════════════════════════════════════════════════════╗"
        echo "║  llama.cpp + Qwen2.5-VL-7B Installer for CachyOS        ║"
        echo "║  Target: /opt/qwen2.5-vl-7b/ (official llama.cpp)       ║"
        echo "╚══════════════════════════════════════════════════════════╝"
    fi
    echo ""
    ...
    if [[ $UPDATE_MODE -eq 1 ]]; then
        ok "═══════════════════════════════════════════════════════════"
        ok "Qwen2.5-VL-7B Update complete!"
        ok "═══════════════════════════════════════════════════════════"
        echo ""
        info "Restart your server to pick up changes:"
        info "  ./scripts/service-switcher.sh vl-7b"
    else
        ok "═══════════════════════════════════════════════════════════"
        ok "Qwen2.5-VL-7B Installation complete!"
        ok "═══════════════════════════════════════════════════════════"
        echo ""
        if [[ $TEST_MODE -eq 1 ]]; then
            info "Test instance:"
            info "  1. Verify:  curl http://localhost:10504/v1/models"
            info "  2. Clean:   sudo systemctl stop --now llama-server-qwen2.5-vl-7b-test"
        else
            info "Next steps:"
            info "  1. Verify:        curl http://localhost:10500/v1/models"
            info "  2. Switch model:  ./scripts/service-switcher.sh vl-7b"
