#!/usr/bin/env bash
# Install script for Gemma 4 26B A4B (llama.cpp, MoE model)
# Uses bartowski's imatrix-calibrated GGUF
# Targets /opt/ for system-wide installation
# Run as: sudo bash setup-26b-a4b.sh [--update|--uninstall|--uninstall-all]
#
...
LLAMA_DIR="/opt/26b-a4b/llama.cpp"
MODELS_DIR="/opt/26b-a4b/models"
HF_TOKEN_ENV=""
if [[ -n "$HF_TOKEN" ]]; then
...
MODEL_NAME="google_gemma-4-26B-A4B-it-Q4_K_L.gguf"
MMPROJ_NAME="mmproj-google_gemma-4-26B-A4B-it-f16.gguf"
MMPROJ_REPO="bartowski/google_gemma-4-26B-A4B-it-GGUF"
...
section_model() {
    info "=== Section 3: Download Gemma 4 26B A4B ==="

    if [[ $UPDATE_MODE -eq 1 ]]; then
        ok "Update mode: skipping main model files"
    else
        mkdir -p "$MODEL_DIR"
        ...
        if [[ -f "$MODEL_DIR/$MODEL_NAME" ]]; then
            ...
            info "Downloading $MODEL_NAME (~17.2 GB, bartowski Q4_K_L imatrix)..."
    ...
    # Always ensure mmproj is present
    if [[ ! -f "$MODEL_DIR/$MMPROJ_NAME" ]]; then
        info "Downloading $MMPROJ_NAME from $MMPROJ_REPO..."
        HF_XET_HIGH_PERFORMANCE=1 HF_TOKEN="$HF_TOKEN" "$HF_CLI" download "$MMPROJ_REPO" "$MMPROJ_NAME" --local-dir "$MODEL_DIR"
    else
        ok "$MMPROJ_NAME already exists."
    fi
...
section_services() {
    ...
    for svc_base in llama-server-26b-a4b; do
        local src="$CONFIGS_DIR/${svc_base}.service"
        if [[ -f "$src" ]]; then
            sed -e "s|/opt/26b-a4b/llama.cpp|$LLAMA_DIR|g" \
                -e "s|/opt/26b-a4b/models|$MODELS_DIR|g" \
                -e "s|--port 10500|--port 10500|g" \
                -e "s|__USERNAME__|$USERNAME|g" \
                "$src" > "/etc/systemd/system/${svc_base}.service"
            ok "Installed /etc/systemd/system/${svc_base}.service"
        else
    ...
    local active_svc="llama-server-26b-a4b"
    info "Starting server (128K context, MoE, thinking mode)..."
    systemctl enable --now "$active_svc"

    if systemctl is-active --quiet "$active_svc"; then
        ok "$active_svc is running on port 10500"
    else
    ...
    info "  ./scripts/service-switcher.sh 26b-a4b    (Gemma 4 26B A4B MoE)"
...
section_uninstall() {
    ...
    info "Stopping 26B A4B service..."
    sudo systemctl stop "llama-server-26b-a4b" 2>/dev/null || true
    sudo systemctl disable "llama-server-26b-a4b" 2>/dev/null || true

    local svc_file="/etc/systemd/system/llama-server-26b-a4b.service"
    if [[ -f "$svc_file" ]]; then
    ...
    info "26B A4B service removed."
    if [[ $UNINSTALL_ALL -eq 1 ]]; then
        info "Removing all 26B A4B data..."
        [[ -d "/opt/26b-a4b" ]] && { info "Removing /opt/26b-a4b"; sudo rm -rf /opt/26b-a4b; }
        info "All 26B A4B data removed."
    else
        info "Models and llama.cpp build kept. Remove manually with --uninstall-all."
    fi

    echo ""
    ok "26B A4B uninstall complete!"
}
...
section_opencode() {
    ...
    info "To use Gemma 4 26B A4B with OpenCode, add this to ~/.config/opencode/opencode.json:"
    info ""
    cat <<'CONFIG'
  "gemma4-a4b": {
    "npm": "@ai-sdk/openai-compatible",
    "name": "Gemma 4 26B A4B (MoE, Q4_K_L)",
    "options": {
...
CONFIG
    info ""
    info 'Then set: "model": "gemma4-a4b/gemma4-26b"'
}
...
main() {
    if [[ $UNINSTALL_MODE -eq 1 ]]; then
        echo "╔══════════════════════════════════════════════════════════╗"
        if [[ $UNINSTALL_ALL -eq 1 ]]; then
            echo "║  Gemma 4 26B A4B Full Uninstaller                       ║"
            echo "║  Removing: service, systemd files, models, build        ║"
        else
            echo "║  Gemma 4 26B A4B Service Uninstaller                    ║"
    ...
    if [[ $UPDATE_MODE -eq 1 ]]; then
        echo "╔══════════════════════════════════════════════════════════╗"
        echo "║  Gemma 4 26B A4B Updater                                ║"
        echo "║  Updating: llama.cpp, huggingface_hub, services         ║"
        echo "╚══════════════════════════════════════════════════════════╝"
    else
        echo "╔══════════════════════════════════════════════════════════╗"
        echo "║  Gemma 4 26B A4B Installer for CachyOS                  ║"
        echo "║  Target: /opt/26b-a4b/ (bartowski Q4_K_L imatrix GGUF) ║"
        echo "╚══════════════════════════════════════════════════════════╝"
    fi
    echo ""
    ...
    if [[ $UPDATE_MODE -eq 1 ]]; then
        ok "═══════════════════════════════════════════════════════════"
        ok "Gemma 4 26B A4B Update complete!"
        ok "═══════════════════════════════════════════════════════════"
        echo ""
        info "Restart your server to pick up changes:"
        info "  ./scripts/service-switcher.sh 26b-a4b"
    else
        ok "═══════════════════════════════════════════════════════════"
        ok "Gemma 4 26B A4B Installation complete!"
        ok "═══════════════════════════════════════════════════════════"
        echo ""
        info "Next steps:"
        info "  1. Verify:        curl http://localhost:10500/v1/models"
        info "  2. Switch model:  ./scripts/service-switcher.sh 26b-a4b"
