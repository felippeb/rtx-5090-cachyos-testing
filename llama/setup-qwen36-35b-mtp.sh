...
#
# Full install script for llama.cpp (Unsloth MTP) + Qwen3.6-35B-A3B-MTP on CachyOS + RTX 5090
# Builds llama.cpp from unslothai/llama.cpp (mtp-clean branch)
# Downloads unsloth/Qwen3.6-35B-A3B-MTP-GGUF (UD-Q4_K_XL)
# Run as: sudo bash setup-qwen36-35b-mtp.sh [--unsloth|--test|--update]
#
...
MODEL_DIR="$MODELS_DIR/qwen3.6-35b-a3b-mtp-unsloth"
MODEL_NAME="Qwen3.6-35B-A3B-UD-Q4_K_XL.gguf"
MMPROJ_NAME="mmproj-F16.gguf"
HF_REPO_MTP="unsloth/Qwen3.6-35B-A3B-MTP-GGUF"
...
section_build() {
    ...
    if [[ -d "$LLAMA_DIR" ]] && [[ -d "$LLAMA_DIR/.git" ]] && [[ $UPDATE_MODE -eq 0 ]]; then
        ...
        WORK_DIR="/tmp/llama-35b-mtp-build"
        NEEDS_CLONE=true
...
section_services() {
    ...
    for svc_base in llama-server-unsloth-35b-mtp llama-server-unsloth-35b-mtp-131k; do
    ...
    local default_svc="llama-server-unsloth-35b-mtp${SERVICE_NAME_SUFFIX}"
    info "Starting 35B-A3B MTP mode (64K context, GPU+CPU hybrid, MTP spec decode, n-max=2)..."
    ...
    if [[ $TEST_MODE -eq 1 ]]; then
        ...
        info "  ./scripts/service-switcher.sh unsloth-35b-mtp         (64K context, MTP, n-max=2)"
        info "  ./scripts/service-switcher.sh unsloth-35b-mtp-131k    (131K context, MTP, n-max=2)"
...
section_opencode() {
    ...
    if command -v opencode &>/dev/null; then
        ...
        info ""
        info "To add the 35B-A3B MTP model to your opencode config:"
        info "  Add a provider entry like this to ~/.config/opencode/opencode.json:"
        info ""
        cat <<'TEMPLATE'
    "llama-35b-mtp": {
      "npm": "@ai-sdk/openai-compatible",
      "name": "llama.cpp | Qwen3.6-35B-A3B MTP (MoE, UD-Q4_K_XL)",
      "options": {
        "baseURL": "http://localhost:10500/v1"
      },
      "models": {
        "qwen3.6-35b-a3b-mtp": {
          "name": "Qwen3.6-35B-A3B MTP (64K)",
...
main() {
    ...
    if [[ $UPDATE_MODE -eq 1 ]]; then
        echo "╔══════════════════════════════════════════════════════════╗"
        echo "║  llama.cpp Unsloth MTP + Qwen3.6-35B-A3B Updater       ║"
        echo "║  Updating: llama.cpp (MTP build), services             ║"
        echo "╚══════════════════════════════════════════════════════════╝"
    elif [[ $TEST_MODE -eq 1 ]]; then
        info "TEST MODE: installing to $LLAMA_DIR on port $SERVER_PORT"
        echo "╔══════════════════════════════════════════════════════════╗"
        echo "║  llama.cpp Unsloth MTP + Qwen3.6-35B-A3B (TEST MODE)   ║"
        echo "║  Target: /opt/llama-test/  Port: 10502                  ║"
        echo "╚══════════════════════════════════════════════════════════╝"
    else
        echo "╔══════════════════════════════════════════════════════════╗"
        echo "║  llama.cpp Unsloth MTP + Qwen3.6-35B-A3B Installer     ║"
    ...
    if [[ $UPDATE_MODE -eq 1 ]]; then
        ok "═══════════════════════════════════════════════════════════"
        ok "35B-A3B MTP Update complete!"
        ok "═══════════════════════════════════════════════════════════"
        info "Restart: ./scripts/service-switcher.sh unsloth-35b-mtp"
    else
        ok "═══════════════════════════════════════════════════════════"
        ...
        if [[ $TEST_MODE -eq 1 ]]; then
            ...
            info "  2. Clean:   sudo systemctl stop --now llama-server-unsloth-35b-mtp-test"
        else
            info "Next steps:"
